...
related:
  ...
  plugin_guide: docs/guides/plugin-development.md
license:
  current: GPL-3.0
  target: MIT
---
...
3. Documentation synchronization is complete:
   - `docs/prds/machNotch.md` checklist/status rows updated
...
**Status:** ✅ Partially addressed — `PluginRegistry.makeBuiltInPlugins()` centralises all plugin instantiation; `AppObjectGraph` is no longer the list owner.

Built-in plugins are instantiated eagerly in `PluginRegistry`. No lazy loading, no conditional activation based on hardware capability, no discovery mechanism. Manageable for built-ins, but precludes dynamic plugin loading required for Phase 9.
...
All as `NotchPlugin` conformances. No `PluginManager` modifications. Each plugin: new folder at `Plugins/BuiltIn/<Name>Plugin/`, register in `AppObjectGraph`, add `PluginID` constant. See `docs/guides/plugin-development.md` for the full plugin authoring guide.
