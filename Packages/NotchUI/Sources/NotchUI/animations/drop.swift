...
public enum StandardAnimations {
    ...
    public static func openWithVelocity(_ velocity: CGFloat) -> Animation {
    ...
    public static let openDuration: Duration = .milliseconds(300)
    ...
    public static let close = Animation.spring(
    ...
    public static let closeShell = close
    /// Estimated settle duration for the close animation
    public static let closeDuration: Duration = .milliseconds(230)
    ...
    public static let timingCurve = Animation.timingCurve(0.16, 1, 0.3, 1, duration: 0.7)

    /// Organic timing curve for the hello animation
    public static let hello = Animation.timingCurve(0.2, 0.8, 0.2, 1, duration: 3.0)

    // MARK: - Content Transitions (decoupled from shell spring)

    /// Content reveal on open — shell leads, content follows with gentle delay
    public static let contentReveal = Animation.easeOut(duration: 0.28).delay(0.08)

    /// Content reveal without built-in delay (delay is handled externally via Task).
    /// Used by NotchPhaseCoordinator which manages the stagger via a delayed Task
    /// instead of a separate withAnimation block, reducing animation transaction overhead.
    public static let contentRevealNoDelay = Animation.easeOut(duration: 0.28)

    /// Content exit on close — fast and decisive, content yanked back into notch.
    public static let contentDismiss = close
