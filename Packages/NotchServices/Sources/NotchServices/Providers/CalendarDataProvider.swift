...
public protocol CalendarDataProviding {
...
public class CalendarDataProvider: CalendarDataProviding {
    private let store = EKEventStore()

    public init() {}

    @MainActor
    public func requestAccess(to type: EKEntityType) async throws -> Bool {
        switch type {
        case .event:
    ...
    public func events(from start: Date, to end: Date, calendars ids: [String]) async -> [EventModel] {
...
extension EventType {
    init(from event: EKEvent) {
        self =
            ...
            ? .birthday : .event(.init(from: event.currentUser?.participantStatus))
    }
}
