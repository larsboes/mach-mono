...
final class ImageProcessingService: ImageProcessingServiceProtocol {
    ...
    func removeBackground(from url: URL) async throws -> URL? {
        ...
        guard
            let tempURL = await temporaryFileStorage.createTempFile(
    ...
    private func convertToFormat(
        _ image: NSImage, format: ImageConversionOptions.ImageFormat, quality: Double
    ) throws -> Data? {
    ...
    private func scaleImage(_ image: NSImage, maxDimension: CGFloat) -> NSImage {
        ...
        guard let dstCG = ciContext.createCGImage(output, from: output.extent, format: .RGBA8, colorSpace: colorSpace)
        else {
            return image
        }
