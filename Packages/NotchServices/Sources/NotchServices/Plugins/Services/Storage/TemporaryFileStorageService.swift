...
public enum TempFileType: Sendable {
...
@MainActor
public final class TemporaryFileStorageService: TemporaryFileStorageServiceProtocol {
    public init() {}

    // MARK: - Public Interface

    /// Creates a temporary file and tracks it for manual cleanup
    public func createTempFile(for type: TempFileType) async -> URL? {
    ...
    public func removeTemporaryFileIfNeeded(at url: URL) {
    ...
    public func createZip(from urls: [URL], suggestedName: String? = nil) async -> URL? {
        let tempDir = URL(fileURLWithPath: NSTemporaryDirectory())
        let uuid = UUID().uuidString
