...
@Observable
@MainActor
public final class BluetoothManager: NSObject, BackgroundServiceRestartable, BluetoothStateServiceProtocol {
    ...
    public init(settings: any NotchSettings) {
    ...
    private func getIconForDevice(name: String, deviceClass: UInt32) -> String? {
        ...
        if name.contains("mouse") && name.contains("magic") {
        ...
        // --- Gamepads ---
        if name.contains("gamepad") || name.contains("controller") || name.contains("joy-con") {
...
public struct BluetoothDevice: Identifiable, Equatable, Sendable {
    ...
    public init(id: String, name: String, batteryLevel: Int?, icon: String, isConnected: Bool) {
        self.id = id
        self.name = name
