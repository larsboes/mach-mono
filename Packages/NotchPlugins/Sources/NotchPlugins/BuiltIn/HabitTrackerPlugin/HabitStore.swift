...
public final class HabitStore {
    ...
    public static let predefinedSymbols: [String] = [
    ...
    public func reorderHabits(from: IndexSet, to: Int) {
        habits.move(fromOffsets: from, toOffset: to)
        save()
    }

    public func toggleCompletion(for habitId: UUID, on date: Date = Date()) {
    ...
    public func isCompleted(habitId: UUID, on date: Date = Date()) -> Bool {
        let startOfDay = Calendar.current.startOfDay(for: date)
    ...
    public func stats(for habitId: UUID) -> HabitStats {
