...
// Only `music` is a real controllable mock — every other service is a no-op stub.
// This avoids depending on ServiceContainer internals.
...
final class MusicPluginTests: XCTestCase {
    ...
    func testDisplayRequestWhenPlaying() {
        mockMusicService.playbackState = PlaybackState(
            bundleIdentifier: "com.apple.Music",
            isPlaying: true
        )
        mockMusicService.isPlayerIdle = false
    ...
    func testNoDisplayRequestWhenIdleAndPaused() {
        mockMusicService.playbackState = PlaybackState(bundleIdentifier: "com.apple.Music")
        // isPlaying defaults to false; isPlayerIdle = true → condition false
        mockMusicService.isPlayerIdle = true
    ...
    func testNowPlayingInfoReflectsServiceState() {
        mockMusicService.currentTrack = TrackInfo(title: "Hello", artist: "Adele", album: "25")
        mockMusicService.playbackState = PlaybackState(
            bundleIdentifier: "com.apple.Music",
            isPlaying: true
        )
...
final class MockMusicService: MusicServiceProtocol {
    ...
    @ObservationIgnored private let _playbackStateSubject = PassthroughSubject<
        PlaybackState, Never
    >()
    ...
    @ObservationIgnored private let _sneakPeekSubject = PassthroughSubject<
        SneakPeekRequest, Never
    >()
